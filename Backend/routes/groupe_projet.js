...
router.post('/', auth.verifyToken, multer,  groupeCtrl.save);                                //Créer un groupe dans la BD
router.post('/:id/addEtudiant',auth.verifyToken, multer,  groupeCtrl.ajoutEtudiant)          //Ajout un étudiant dans le groupe identifier avec son "id" (ajout une ligne dans la table composer)
router.put('/:id', auth.verifyToken, multer, groupeCtrl.update);                             //Modifier un groupe représenter par son id
router.delete('/:id', auth.verifyToken, groupeCtrl.delete);                                  //Supprimer un groupe représenter par son id
router.delete('/:id/removeEtudiant',auth.verifyToken, multer,  groupeCtrl.removeEtudiant)    //Supprimer un étudiant dans le groupe identifier avec son "id" (supprime une ligne dans la table composer)
